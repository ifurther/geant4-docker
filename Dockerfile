...
ARG build_G4Version="11.2.p01"
ARG build_shortG4version="11.2.1"
...
RUN if [ ! -e geant4.${shortG4version} ] ; then wget https://geant4-data.web.cern.ch/releases/geant4-v${shortG4version}.tar.gz; \
tar zxvf geant4-v${shortG4version}.tar.gz -C ${G4WKDIR}; \
mv geant4-v${shortG4version}.tar.gz /app/src; fi
...
-DGEANT4_USE_QT=ON -DGEANT4_USE_SYSTEM_ZLIB=ON -DGEANT4_USE_SYSTEM_EXPAT=ON ${G4WKDIR}/geant4-v${shortG4version} &&\
